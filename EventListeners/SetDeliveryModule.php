 ...
 * @package DpcClassic\EventListeners
...
class SetDeliveryModule implements EventSubscriberInterface
{
    ...
    {
        return $id == DpdClassic::getModuleId();
    }

    public function isModuleDpcClassic(OrderEvent $event)
    {
        $address = DpdclassicAddressQuery::create()
        ...
        if ($this->check_module($event->getDeliveryModule())) {
            ...
            if (!empty($pr_code)) {
                ...
                $request->getSession()->set('DpcClassicDeliveryId', $event->getDeliveryAddress());
                if ($address === null) {
                    $address = new DpdclassicAddress();
                ...
                throw new \ErrorException("No pick-up & go store chosen for DpcClassic delivery module");
            }
        } elseif (null !== $address) {
            $address->delete();
        }
    ...
    public function updateDeliveryAddress(OrderEvent $event)
    {
        if ($this->check_module($event->getOrder()->getDeliveryModuleId())) {
            $request = $this->getRequest();
            $tmp_address = DpdclassicAddressQuery::create()
                ->findPk($request->getSession()->get('DpcClassicDeliveryId'));

            if ($tmp_address === null) {
                throw new \ErrorException("Got an error with DpcClassic module. Please try again to checkout.");
            }

            $savecode = new DpdclassicOrderAddress();
    ...
    {
        return array(
            TheliaEvents::ORDER_SET_DELIVERY_MODULE => array('isModuleDpcClassic', 64),
