...
class LabelController extends AdminController
{
    public function showAction()
    {
        ...
        return $this->render("dpdclassic-labels", [
            "err" => $err
        ]);
    }
    ...
    public function saveAction()
    {
        ...
        $labelDir = DpdClassic::DPD_LABEL_DIR;
        ...
        if (!$label = DpdclassicLabelsQuery::create()->filterByOrderId($order->getId())->findOne()) {
            ...
            return $this->generateRedirect(URL::getInstance()->absoluteUrl('admin/module/DpdClassic/labels', $params));
    ...
    public function deleteLabelAction()
    {
        ...
        $label = DpdclassicLabelsQuery::create()->filterByOrderId($orderId)->findOne();

        $fs = new Filesystem();

        $fs->remove($labelDir.DS.$label->getOrder()->getRef().".pdf");
    ...
    protected function createLabel(Order $order, $labelName, $weight)
    {
        ...
        if(1 === (int)DpdClassic::getConfigValue(DpdClassic::API_IS_TEST)){
            $DpdWSD = DpdClassic::DPD_WSDL_TEST;
        }
        ...
        if (false === @file_put_contents($labelName, $labels[0]->label)) {
            return Translator::getInstance()->trans("L'étiquette n'a pas pu être sauvegardée dans $labelName", DpdClassic::DOMAIN_NAME);
        }

        $label = new DpdclassicLabels();
        $label
